function Desktop({ onIconClick, onRecycleBinClick, onMyComputerClick }) {
    ...
    // Side icons (My Computer, Recycle Bin)
    const sideIcons = [
        {
            id: 'mycomputer',
            label: 'My Computer',
            icon: (
                <svg width="64" height="64" viewBox="0 0 48 48" className="pixelated drop-shadow-lg">
    ...
    return (
        <div className="absolute inset-0 p-4">
            {/* Side Icons - Left aligned */}
            <div className="flex flex-col items-start gap-4">
                {sideIcons.map((icon) => (
                    <div
                        key={icon.id}
                        className="desktop-icon-container flex flex-col items-center justify-start p-2 rounded select-none cursor-pointer w-[100px]"
            ...
            {/* Fatih.exe - Center of screen */}
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div
                    className="desktop-icon-container flex flex-col items-center justify-center p-3 rounded select-none cursor-pointer pointer-events-auto"
                ...
                >
                    ...
                    <span
                        className="text-white text-base font-bold text-center leading-tight mt-2"
                        style={{
                            textShadow: '1px 1px 0 #000, -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000'
                        }}
                    >
